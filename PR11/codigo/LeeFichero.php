...
<body>
    ...
    <?php
        ...
        if(sizeof($_REQUEST) > 0)
        {
            ...
            if((isset($_REQUEST["boton"]) &&(!empty($_REQUEST["contenidoFichero"]) &&(existeFichero($_REQUEST['inputNombreFichero'])))))
            {
                ...
                // Volver a la página de Edición
                if($_REQUEST['boton'] == "Volver a Editar")
                {
                    // Abro la página de leer y le paso el nombre del fichero
                    header("Location: EditaFichero.php?nombreFichero=" . $_REQUEST["inputNombreFichero"]);
                }
            }
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="formularioEditar" id="idFormularioEditar">
        ...
        <p>
            ...
            <textarea name="contenidoFichero" id="idContenidoFichero" size="" placeholder="Contenido del Fichero" readonly><?php
        ...
        <input type="submit" value="Volver a Editar" name="boton">
